...
selected_department_label = controls[0].selectbox("??????? ?????????", list(department_labels.keys()))
edge_limit = controls[1].slider("????????? ??'?????", min_value=30, max_value=240, value=120, step=10)
...
summary_columns[0].metric("????? Teacher", teacher_count)
summary_columns[1].metric("????? Publication", publication_count)
