...
def _render_selftest_tab(service) -> None:
    ...
    render_adaptive_dataframe(results_frame, use_container_width=True, hide_index=True, height=280, compact=True)
    st.download_button(
...
def _render_audit_tab(service) -> None:
    ...
    with top[0]:
        ...
        render_adaptive_dataframe(audit_frame, use_container_width=True, hide_index=True, height=340, compact=True)
        st.download_button(
            "Експорт аудиту CSV",
...
def _render_duplicate_candidates(service, all_publications: list[dict[str, object]]) -> None:
    ...
    with top[0]:
        ...
        render_adaptive_dataframe(duplicate_frame, use_container_width=True, hide_index=True, height=300, compact=True)
    with top[1]:
        render_key_value_card(
            "Огляд дублювання",
            [
    ...
    with moderation_tab:
        ...
        with moderation_layout[0]:
            if filtered_problematic:
                ...
                render_adaptive_dataframe(problem_frame, use_container_width=True, hide_index=True, height=340, compact=True)
        ...
        with teacher_sections[0]:
            ...
            if without_profiles_frame.empty:
                ...
                render_adaptive_dataframe(without_profiles_frame, use_container_width=True, hide_index=True, height=280, compact=True)
        ...
        with teacher_sections[1]:
            ...
            if without_publications_frame.empty:
                ...
                render_adaptive_dataframe(without_publications_frame, use_container_width=True, hide_index=True, height=280, compact=True)
