...
def render() -> None:
    ...
    if counts["publications"] == 0:
        st.info(
            "Структура вже заведена. Для наповнення бази відкрийте `Структура`: основним джерелом можуть бути відкриті сторінки ХДУ "
            "з публікаціями у Scopus та Web of Science, а зовнішні API-сервіси використовуються як додаткове збагачення."
        )
    else:
